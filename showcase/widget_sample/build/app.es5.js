            ...
            var _loop = function _loop() {
                ...
                if (dirName === 'drawable') {
                    var findImageWithRatioName = function findImageWithRatioName(ratio) {
                        ...
                        if (!netImage) {
                            var _fileStr = dir[fileNameWithNinePatch];
                            if (_fileStr && _fileStr.startsWith('data:image')) {
                                netImage = new NetImage(_fileStr, ratio);
                                imageFileCache.set(key, netImage);
                            }
                        }
                ...
                netImage = imageFileCache.get(key);
                if (!netImage) {
                    var _fileStr2 = dir[fileNameWithNinePatch];
                    if (_fileStr2 && _fileStr2.startsWith('data:image')) {
                        netImage = new NetImage(_fileStr2, ratio);
                        imageFileCache.set(key, netImage);
                    }
                }
                if (netImage) return {
        ...
        var SampleBaseWidgetActivity = function (_ActionBarActivity3) {
            ...
            _createClass(SampleBaseWidgetActivity, [{
                ...
                value: function onCreate() {
                    ...
                    var dialog = new AlertDialog.Builder(activity).setTitle('Title').setMessage('ContentContent').setPositiveButton(android.R.string_.ok, {
                        onClick: function onClick(dialog, which) {
                            Toast.makeText(activity, android.R.string_.ok, Toast.LENGTH_SHORT).show();
                        }
                    }).setIcon(R.drawable.icon_alert).setNegativeButton(android.R.string_.cancel, null).create();
